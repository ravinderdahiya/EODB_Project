...
class Shell extends LitElement {
  ...
  handleAlertsSlotChange(event) {
    ...
    slotChangeGetAssignedElements(event).filter((el) => el?.matches("calcite-alert")).forEach((el) => {
      el.embedded = true;
    });
  }
  handleSheetsSlotChange(event) {
    this.hasSheets = !!slotChangeHasAssignedElement(event);
    slotChangeGetAssignedElements(event).filter((el) => el?.matches("calcite-sheet")).forEach((el) => {
      el.embedded = true;
    });
  }
  handleModalsSlotChange(event) {
    this.hasModals = !!slotChangeHasAssignedElement(event);
    slotChangeGetAssignedElements(event)?.filter((el) => el?.matches("calcite-modal")).forEach((el) => {
      el.embedded = true;
    });
  }
  handlePanelTopChange(event) {
    this.hasPanelTop = slotChangeHasAssignedElement(event);
    slotChangeGetAssignedElements(event).filter((el) => el?.matches("calcite-shell-panel")).forEach((el) => {
  ...
  handleDialogsSlotChange(event) {
    ...
    slotChangeGetAssignedElements(event).filter((el) => el?.matches("calcite-dialog")).forEach((el) => {
      el.embedded = true;
    });
  }
  renderHeader() {
  ...
  renderContent() {
    ...
    const content = this.contentBehind ? [
      ...
      })}>${panelTopSlotNode}${panelBottomSlotNode}</div>`
    ] : [
      keyed(contentContainerKey, html`<div class=${safeClassMap({ [CSS.content]: true, [CSS.contentBottom]: this.hasOnlyPanelBottom })}>${panelTopSlotNode}${defaultSlotContainerNode}${panelBottomSlotNode}</div>`)
    ];
    return content;
  }
  renderMain() {
    return html`<div class=${safeClassMap(CSS.main)}><slot name=${SLOTS.panelStart} @slotchange=${this.handlePanelStartChange}></slot>${this.renderContent()}<slot name=${SLOTS.panelEnd} @slotchange=${this.handlePanelEndChange}></slot></div>`;
  }
