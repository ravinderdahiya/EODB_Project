...
class Tile extends LitElement {
  ...
  renderTile() {
    const { description, disabled, hasContentBottom, hasContentTop, heading, headingLevel, icon, iconFlipRtl, interactive, selectionMode } = this;
    ...
    })}><slot name=${SLOTS.contentTop} @slotchange=${this.handleSlotChange}></slot>${icon && html`<calcite-icon class=${safeClassMap(CSS.icon)} .flipRtl=${iconFlipRtl} .icon=${icon} scale=l></calcite-icon>` || ""}<div class=${safeClassMap({ [CSS.textContentContainer]: true, [CSS.row]: true })}><div class=${safeClassMap(CSS.textContent)}>${heading && Heading({ class: CSS.heading, level: headingLevel, children: heading }) || ""}${description && html`<div class=${safeClassMap(CSS.description)}>${description}</div>` || ""}</div></div><slot name=${SLOTS.contentBottom} @slotchange=${this.handleSlotChange}></slot></div></div>`;
  }
  render() {
    const { disabled } = this;
    return this.interactiveContainer({ disabled, children: this.href ? html`<calcite-link .disabled=${disabled} .href=${this.href}>${this.renderTile()}</calcite-link>` : this.renderTile() });
  }
}
