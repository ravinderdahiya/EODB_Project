...
class InputTimeZone extends LitElement {
  ...
  overrideSelectedLabelForRegion(open) {
    ...
    this.comboboxRef.value.selectedItems[0].heading = this.getItemLabel(this.selectedTimeZoneItem, open);
  }
  ...
  renderItems() {
    ...
    return repeat(this.timeZoneItems, ({ label }) => label, (group) => {
      ...
      return html`<calcite-combobox-item data-label=${label ?? nothing} .heading=${label} .metadata=${metadata} .selected=${selected} .value=${value}></calcite-combobox-item>`;
    });
  }
  renderRegionItems() {
    return this.timeZoneItems.flatMap(({ label, items }) => keyed(label, html`<calcite-combobox-item-group .label=${label}>${repeat(items, ({ label: label2 }) => label2, (item) => {
      ...
      return html`<calcite-combobox-item data-label=${label2 ?? nothing} .description=${metadata.country} .heading=${heading} .metadata=${metadata} .selected=${selected} .value=${value}><span class=${safeClassMap(CSS.offset)} slot=content-end>${metadata.offset}</span></calcite-combobox-item>`;
