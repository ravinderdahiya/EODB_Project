...
import { B as filterDirectChildren, b as slotChangeGetAssignedElements, s as slotChangeHasAssignedElement, g as getElementDir, t as toAriaBoolean } from "../../chunks/dom.js";
...
class TreeItem extends LitElement {
  constructor() {
    super();
    this.actionSlotWrapperRef = createRef();
  ...
  getSelectionIcon() {
    ...
    if (selectionMode === "single" || selectionMode === "children" || selectionMode === "single-persist") {
  ...
  render() {
    ...
    const chevron = this.hasChildren || this.selectionMode === "ancestors" ? html`<calcite-icon class=${safeClassMap({
      [CSS.chevron]: true,
      [CSS_UTILITY.rtl]: rtl
    })} .icon=${this.hasChildren ? ICONS.chevronRight : ICONS.blank} @click=${this.iconClickHandler} .scale=${getIconScale(this.scale)}></calcite-icon>` : null;
    ...
    const itemIndicator = selectionIcon ? html`<calcite-icon class=${safeClassMap({
      [CSS.selectionIcon]: true,
      [CSS_UTILITY.rtl]: rtl
    })} .icon=${selectionIcon} .scale=${getIconScale(this.scale)}></calcite-icon>` : null;
    ...
    return this.interactiveContainer({ disabled: this.disabled, children: html`<div class=${safeClassMap({ [CSS.itemExpanded]: isExpanded })}><div class=${safeClassMap(CSS.nodeAndActionsContainer)}><div class=${safeClassMap({
      [CSS.nodeContainer]: true,
      [CSS_UTILITY.rtl]: rtl
    })} data-selection-mode=${this.selectionMode ?? nothing}>${chevron}${itemIndicator}${checkbox ? checkbox : null}${this.iconStart ? iconStartEl : null}${checkbox ? html`<label class=${safeClassMap(CSS.checkboxLabel)}>${defaultSlotNode}</label>` : defaultSlotNode}</div><div class=${safeClassMap(CSS.actionsEnd)} .hidden=${!hasEndActions} ${ref(this.actionSlotWrapperRef)}>${slotNode}</div></div><div class=${safeClassMap({
      [CSS.childrenContainer]: true,
      [CSS_UTILITY.rtl]: rtl
    })} @click=${this.childrenClickHandler} .role=${this.hasChildren ? "group" : void 0}><slot name=${SLOTS.children} @slotchange=${this.handleChildrenSlotChange}></slot></div></div>` });
  }
}
