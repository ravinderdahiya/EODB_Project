...
import { y as intersects } from "../../chunks/dom.js";
...
class Meter extends LitElement {
  constructor() {
    super(...arguments);
    this.highLabelRef = createRef();
    this.labelFlipMax = 0.8;
    this.labelFlipProximity = 0.15;
    this.lowLabelRef = createRef();
    this.maxLabelRef = createRef();
    this.maxPercent = 100;
    this.messages = useT9n({ name: null });
    this.meterContainerRef = createRef();
    this.minLabelRef = createRef();
    this.minPercent = 0;
  ...
  renderValueLabel() {
    ...
    return keyed("low-label-line", html`<div class=${safeClassMap({ [CSS.label]: true, [CSS.labelValue]: true })} ${ref(this.valueLabelRef)}>${label}${unitLabel && valueLabelType !== "percent" && html`<span class=${safeClassMap(CSS.unitLabel)}>&nbsp;${unitLabel}</span>` || ""}</div>`);
  }
  renderMinLabel() {
    ...
    return keyed("min-label-line", html`<div class=${safeClassMap({ [CSS.label]: true, [CSS.labelRange]: true })} style=${safeStyleMap(style)} ${ref(this.minLabelRef)}>${labelMin}${unitLabel && rangeLabelType !== "percent" && html`<span class=${safeClassMap(CSS.unitLabel)}>&nbsp;${unitLabel}</span>` || ""}</div>`);
  }
  renderLowLabel() {
    ...
    return keyed("low-label-line", html`<div class=${safeClassMap({ [CSS.label]: true, [CSS.labelRange]: true })} style=${safeStyleMap(style)} ${ref(this.lowLabelRef)}>${label}</div>`);
  }
  renderHighLabel() {
    ...
    return keyed("high-label-line", html`<div class=${safeClassMap({ [CSS.label]: true, [CSS.labelRange]: true })} style=${safeStyleMap(style)} ${ref(this.highLabelRef)}>${label}</div>`);
  }
  renderMaxLabel() {
    ...
    return keyed("max-label-line", html`<div class=${safeClassMap({ [CSS.label]: true, [CSS.labelRange]: true })} style=${safeStyleMap(style)} ${ref(this.maxLabelRef)}>${labelMax}</div>`);
  }
  render() {
