...
class TableRow extends LitElement {
  constructor() {
    super();
    this.rowCells = [];
    this.tableRowSlotRef = createRef();
  ...
  render() {
    return this.interactiveContainer({ disabled: this.disabled, children: html`<tr .ariaRowIndex=${this.positionAll + 1} .ariaSelected=${this.selected} class=${safeClassMap({ [CSS.lastVisibleRow]: this.lastVisibleRow })} @keydown=${this.keyDownHandler} ${ref((el) => {
      if (!el) {
        return;
      }
      this.tableRowEl = el;
      render(html`${this.numbered && this.renderNumberedCell() || ""}${this.selectionMode !== "none" && this.renderSelectableCell() || ""}<slot ${ref(this.tableRowSlotRef)}></slot>`, el);
    })}></tr>` });
  }
}
