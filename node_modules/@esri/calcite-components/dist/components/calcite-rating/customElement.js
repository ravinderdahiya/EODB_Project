...
class Rating extends LitElement {
  ...
  render() {
    ...
    return this.interactiveContainer({ disabled: this.disabled, children: html`<span class=${safeClassMap(CSS.wrapper)}>${this.labelText && InternalLabel({ labelText: this.labelText, onClick: this.onLabelClick, required: this.required, tooltipText: this.messages.required }) || ""}<fieldset class=${safeClassMap(CSS.fieldSet)} .disabled=${this.disabled}><legend class=${safeClassMap(CSS.visuallyHidden)}>${this.messages.rating}</legend>${this.starsMap.map(({ average, checked, fraction, hovered, id, partial, selected, value, tabIndex }) => {
