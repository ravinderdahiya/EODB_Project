...
class Pagination extends LitElement {
  ...
  renderEllipsis(type) {
    return keyed(type, html`<span class=${safeClassMap(CSS.ellipsis)}>&hellip;</span>`);
  }
  renderItems() {
  ...
  renderPreviousChevron() {
    ...
    })} .disabled=${disabled} @click=${this.previousClicked}><calcite-icon flip-rtl .icon=${ICONS.previous} .scale=${getIconScale(this.scale)}></calcite-icon></button>`);
  }
  renderNextChevron() {
    ...
    return keyed("next-button", html`<button .ariaLabel=${messages.next} class=${safeClassMap({
      [CSS.chevron]: true,
      [CSS.disabled]: disabled
    })} .disabled=${disabled} @click=${this.nextClicked}><calcite-icon flip-rtl .icon=${ICONS.next} .scale=${getIconScale(this.scale)}></calcite-icon></button>`);
  }
  renderFirstChevron() {
