...
const ASMX_BASE = "/LandOwnerAPI/getownername.asmx";

// ─── Internal XML parse helper ────────────────────────────────────────────────
/**
...
export async function getKhewats(dCode, tCode, vCode) {
  ...
  const nodes = doc.getElementsByTagName("khewat");
...
export async function getKhatonis(dCode, tCode, vCode, period, khewat) {
  ...
  const nodes = doc.getElementsByTagName("khatoni");
  return Array.from(nodes)
    .map((n) => n.firstChild?.nodeValue?.trim())
    .filter(Boolean);
}
